...
public class HqlQueries {
    ...
    public static final String GET_THE_NAME_AND_NUMBER_OF_PRESCRIBED_MEDICINES_OF_EACH_PATIENT_HQL = "select p.name, count(pm.id) from MedicalRecord mr join Patient p on mr.patientId = p.id join mr.prescribedMedication pm group by p.id, p.name";
