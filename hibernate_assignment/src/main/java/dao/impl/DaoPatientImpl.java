...
public class DaoPatientImpl implements DaoPatient {
    ...
    public Either<AppError, Integer> save(Patient patient) {
        ...
        try {
            ...
            Patient patientWithoutCredential = new Patient(patient.getId(), patient.getName(), patient.getBirthDate(), patient.getPhone());

            tx.begin();

            em.persist(patientWithoutCredential);
            em.flush(); //this will make the ID available

            //now we get the patientId and put it in the credential
            int patientId = patientWithoutCredential.getId();
            credential.setPatientId(patientId);
